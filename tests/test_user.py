...
class UsersTest(BaseTests):
    ...
    def test_admin_get_user(self):
        ...
        response = self.app.get('/api/v1/users/1', headers=self.admin_header)
    ...
    def test_good_deletion(self):
        ...
        response = self.app.delete('/api/v1/users/1', headers=self.admin_header)
        self.assertEqual(response.status_code, 200)
    
    def test_delete_non_existing(self):
        """Test a deleting user that does not exist"""
        response = self.app.delete('/api/v1/users/5', headers=self.admin_header)
        self.assertEqual(response.status_code, 404)
